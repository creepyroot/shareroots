...
function useTransfer() {
  ...
  const [rootId, setRootId] = (0, _react.useState)('');
  ...
  const metadataRef = (0, _react.useRef)(null);
  const [currentIndex, setCurrentIndex] = (0, _react.useState)(0);
  const [totalFiles, setTotalFiles] = (0, _react.useState)(0);
  const peerRef = (0, _react.useRef)(null);
  const connRef = (0, _react.useRef)(null);
  const filesRef = (0, _react.useRef)([]);

  // Receiver state
  const receivedBytesRef = (0, _react.useRef)(0);
  const fileStreamRef = (0, _react.useRef)(null); // FileSystemWritableFileStream
  const dirHandleRef = (0, _react.useRef)(null); // FileSystemDirectoryHandle
  const chunksCacheRef = (0, _react.useRef)([]); // Fallback if no FileSystem API
  ...
  // SENDER
  const startHosting = (0, _react.useCallback)(files => {
    cleanup();
    ...
    const peer = new _peerjs.default(fullId, {
      debug: 2,
      config: {
  ...
  // RECEIVER
  const connectToRoot = (0, _react.useCallback)(code => {
    cleanup();
  ...
  const resumeDownload = (0, _react.useCallback)(() => {
    if (rootId) {
      connectToRoot(rootId);
    }
  }, [rootId, connectToRoot]);
  const acceptTransfer = (0, _react.useCallback)(async () => {
