...
import { FileMetadata, SignalMessage, TransferStatus } from '../types';
...
export function useTransfer() {
  ...
  const fileStreamRef = useRef<any>(null); // FileSystemWritableFileStream
  const dirHandleRef = useRef<any>(null); // FileSystemDirectoryHandle
  const chunksCacheRef = useRef<ArrayBuffer[]>([]); // Fallback if no FileSystem API
  ...
  const cleanup = useCallback(() => {
    ...
    if (fileStreamRef.current) {
      try { fileStreamRef.current.close(); } catch(e) {}
  ...
  // SENDER
  const startHosting = useCallback((files: File[]) => {
    cleanup();
    ...
    peer.on('connection', (conn) => {
      ...
      const sendNextFile = () => {
         ...
         if (currentFileIdx >= filesRef.current.length) {
            connRef.current.send({ type: 'all-complete' });
            setStatus('completed');
            setProgress(100);
            return;
         }
      ...
      conn.on('data', async (data: unknown) => {
        ...
        if (msg.type === 'accept') {
          ...
          while (offset < file.size) {
            ...
            try {
              conn.send({ type: 'file-data', buffer: arrayBuffer });
             ...
             const autoAcceptNextFile = async () => {
                 ...
                 try {
                     if (dirHandleRef.current) {
                         const fileHandle = await dirHandleRef.current.getFileHandle(msg.metadata.name, { create: true });
                 ...
                 setStatus('transferring');
                 conn.send({ type: 'accept', startOffset: 0 });
             };
             autoAcceptNextFile();
          }
        }
  ...
  const acceptTransfer = useCallback(async () => {
    ...
    if ('showDirectoryPicker' in window && totalFiles > 1 && !dirHandleRef.current) {
       try {
           // @ts-ignore
           dirHandleRef.current = await window.showDirectoryPicker({ mode: 'readwrite' });
       } catch (err) {
