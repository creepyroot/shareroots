...
export default function Sender({
  ...
  const renderStatus = () => {
    ...
    if (status === 'error') {
      return <div className="text-center p-8 bg-red-500/10 border border-red-500/50 rounded-2xl w-full">
          <p className="text-red-400 font-medium mb-4">{errorMsg || "An unexpected error occurred."}</p>
          <button onClick={() => {
          transferManager.cleanup();
          setFiles([]);
        }} className="text-slate-300 hover:text-white underline">Start Over</button>
        </div>;
    }
    if (status === 'generating_id') {
      return <div className="flex flex-col items-center justify-center p-12 text-slate-400">
      ...
      return <motion.div initial={{
        ...
        scale: 1
           ...
           <div className="flex items-center justify-between bg-slate-950 border border-slate-800 rounded-xl p-4 mb-6">
              ...
              <button onClick={handleCopyCode} className="p-2 bg-slate-800 hover:bg-slate-700 rounded-lg transition-colors" title="Copy Code">
                {copied ? <Check className="w-5 h-5 text-emerald-400" /> : <Copy className="w-5 h-5 text-slate-400" />}
              </button>
           </div>
           
           <div className="flex flex-col gap-3">
              <button onClick={handleCopyLink} className="w-full flex items-center justify-center gap-2 py-3 px-4 bg-red-600 hover:bg-red-500 text-white font-medium rounded-xl transition-colors red-glow">
           ...
           <div className="mt-8 flex flex-col items-center gap-3">
             <div className="relative flex items-center justify-center w-12 h-12">
               <div className="absolute inset-0 rounded-full border-2 border-red-500/30 border-t-red-400 animate-spin"></div>
             </div>
        ...
        scale: 1
          ...
          <div className="flex justify-between items-center mb-6">
           ...
           <div className="w-full h-2 bg-slate-950 rounded-full overflow-hidden">
              <motion.div className="h-full bg-red-500 red-glow" initial={{
            width: 0
          }} animate={{
            width: `${progress}%`
          }} transition={{
  ...
  return <div className="flex flex-col items-center justify-center min-h-[80vh] px-4 w-full">
      ...
      <AnimatePresence mode="wait">
        {files.length === 0 ? <motion.div key="upload" initial={{
        opacity: 0,
        y: 20
      }} animate={{
        opacity: 1,
        ...
        scale: 0.95
            ...
            <div onDragOver={handleDrag} onDragEnter={handleDrag} onDragLeave={handleDrag} onDrop={handleDrop} onClick={() => fileInputRef.current?.click()} className="border-2 border-dashed border-slate-700 hover:border-red-500/50 rounded-3xl p-16 text-center cursor-pointer bg-slate-900/20 hover:bg-slate-900/40 backdrop-blur-sm transition-all duration-300 group">
              ...
              <div className="inline-flex items-center gap-2 bg-slate-950 border border-slate-800 px-4 py-2 rounded-lg text-sm text-slate-300 font-mono">
                Initiate Secure Channel <ArrowRight className="w-4 h-4 text-red-500 group-hover:translate-x-1 transition-transform" />
              </div>
            </div>
          </motion.div> : <motion.div key="status" initial={{
        opacity: 0,
        y: 20
      }} animate={{
        opacity: 1,
