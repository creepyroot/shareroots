...
export default function Receiver({ onBack, transferManager, initialCode }: ReceiverProps) {
  ...
  const renderStatus = () => {
    const { status, progress, metadata, errorMsg } = transferManager;
    
    if (status === 'error') {
      return (
        <div className="text-center p-8 bg-red-500/10 border border-red-500/50 rounded-2xl w-full max-w-md mx-auto">
    ...
    if (status === 'waiting_for_approval') {
       return (
        ...
        >
          <div className="absolute top-0 inset-x-0 h-1 bg-gradient-to-r from-indigo-500 via-purple-500 to-indigo-500 opacity-50" />
          <div className="flex flex-col items-center justify-center text-center">
            ...
            <button
               ...
               className="w-full flex items-center justify-center gap-2 py-4 px-4 bg-indigo-600 hover:bg-indigo-500 text-white font-medium rounded-xl transition-all shadow-[0_0_24px_-6px_rgba(99,102,241,0.4)]"
            >
               Accept & Download
            </button>
          </div>
      ...
      return (
        ...
        >
          ...
          <div className="flex justify-between items-center mb-6">
            ...
            <span className="text-indigo-400 font-mono text-lg">{progress.toFixed(0)}%</span>
          </div>
          
          <div className="bg-slate-950 p-4 rounded-xl border border-slate-800 mb-6 flex flex-col gap-1">
             <div className="flex justify-between w-full mb-1">
               <span className="text-xs text-indigo-500 font-bold tracking-wider uppercase">
          ...
          <div className="w-full h-2 bg-slate-950 rounded-full overflow-hidden">
             <motion.div 
               className="h-full bg-indigo-500 shadow-[0_0_24px_-6px_rgba(99,102,241,0.5)]"
               initial={{ width: 0 }}
          ...
          {!('showSaveFilePicker' in window) && status === 'transferring' && transferManager.totalFiles === 1 && (
             <p className="text-[10px] text-orange-400/80 mt-4 text-center">
          ...
          <form onSubmit={handleSubmit}>
            <div className="relative mb-6">
              <input
                ...
                placeholder="XXXXXX"
                className="w-full bg-slate-950 border-2 border-slate-800 rounded-xl px-6 py-4 text-center text-3xl font-mono tracking-[0.5em] text-white placeholder-slate-700 focus:outline-none focus:border-indigo-500/50 transition-colors uppercase"
                maxLength={6}
              />
            </div>
            ...
            <button
               ...
               className="w-full flex items-center justify-center gap-2 py-4 bg-indigo-600 hover:bg-indigo-500 disabled:opacity-50 disabled:cursor-not-allowed text-white font-medium rounded-xl transition-all hover:shadow-[0_0_24px_-6px_rgba(99,102,241,0.4)]"
            >
